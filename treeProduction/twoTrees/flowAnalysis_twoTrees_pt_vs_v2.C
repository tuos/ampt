...
#include <TMath.h>
using namespace std; 
void flowAnalysis_twoTrees_pt_vs_v2(){
     ...
     TFile inFile("./trees_hadrons_participants_jb0_0.root", "read");
     ...
     for(long ne=0; ne<nEvt; ne++)
     {
       ...
       for(iParticle=0; iParticle<nParticle; iParticle++){
         ...
         if(eta[iParticle]>0){
           ...
           histV2PP[ptBin]->Fill(cos(2*(phi[iParticle] - Psipart)));
          }
         if(eta[iParticle]<0){
      ...
      ofstream output_ptv2PP;
      output_ptv2EP.open("output_v2ptEP.txt");
      output_ptv2PP.open("output_v2ptPP.txt");
      Float_t error;
      for(int i=0; i<NB_SLICE_PT; i++){     
       ...
       output_ptv2PP<<(minPtCut[i]+maxPtCut[i])/2<<"   "<<histV2PP[i]->GetMean()<<"   "<<histV2PP[i]->GetMeanError()<<endl;
       } 
