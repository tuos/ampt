...
void flowAnalysis_npart(){
     ...
     const int NB_SLICE_PT = 9;
     double minPtCut[NB_SLICE_PT] = {0,50,100,150,200,250,300,350,400};
     double maxPtCut[NB_SLICE_PT] = {50,100,150,200,250,300,350,400,450}; //9 bins
     ...
     for(long ne=0; ne<nEvt; ne++)
     {
       ...
       int NpartBin = -1;
       for(int i=0; i<NB_SLICE_PT; i++){
           if((Npart>minPtCut[i] && Npart<=maxPtCut[i])){
             NpartBin = i;
           }         
       }
       histEventPlaneRes[NpartBin]->Fill(cos(2*(eventPlanePlus - eventPlaneMinus)));
     ...
     for(long ne=0; ne<nEvt; ne++)
     {
       ...
       for(iParticle=0; iParticle<nParticle; iParticle++){
         ...
         if(eta[iParticle]>0){
           histV2Obs[NpartBin]->Fill(cos(2*(phi[iParticle] - eventPlaneMinus)));
           histV2Corrected[NpartBin]->Fill(cos(2*(phi[iParticle] - eventPlaneMinus)));
         }
         if(eta[iParticle]<0){
           histV2Obs[NpartBin]->Fill(cos(2*(phi[iParticle] - eventPlanePlus)));
           histV2Corrected[NpartBin]->Fill(cos(2*(phi[iParticle] - eventPlanePlus)));
         }
      ...
      for(int i=0; i<NB_SLICE_PT; i++){     
        ...
        output_v2<<(minPtCut[i]+maxPtCut[i])/2<<"   "<<histV2Corrected[i]->GetMean()/sqrt(fabs(histEventPlaneRes[i]->GetMean()))<<"   "<<histV2Corrected[i]->GetMeanError()<<endl;
      } 
