...
void hadron_readout2tree(){
   ...
   const string inputFileName = "/scratch/tuos/models/ampt/Ampt-v1.26t9b-v2.26t9b/ana/ampt.dat";
   ifstream inputFile;
   inputFile.open(inputFileName);

   TFile *hadronTreeFile = new TFile("tree_hadrons_ampt.root", "RECREATE");
   ...
   while (inputFile>>nEvent){
     ...
     for(int i=0; i<nMultiplicity; i++){
       ...
       if(particles.Perp()<0.01) continue;
